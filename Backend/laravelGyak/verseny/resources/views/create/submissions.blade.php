...
@section('content')
        <form action="{{ route('store.submission') }}" method="POST">
                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="col-5">
                            ...
                            <select id="challange_id" name="challange_id" class="form-select @error('challange_id') is-invalid @enderror">
                                @foreach($challanges as $challange)
                                    <option value="{{$challange->id}}" {{ old('challange_id') == $challange->id ? 'selected' : '' }}>{{ $challange->title }}</option>
                                @endforeach
                            </select>
            ...
            <div class="mt-3 d-flex justify-content-center gap-2">
                <a class="btn btn-outline-secondary" href="{{ route('dashboard.submissions') }}"><i class="bi bi-arrow-left"></i> Mégse</a>
